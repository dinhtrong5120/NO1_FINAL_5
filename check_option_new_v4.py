...
def check_option_new_v4(df_karen2, data_spec, df_input, dict_input):
    ...
    flag_check_up = (df_input.iloc[2] == '最上級').any()
    flag_check_down = (df_input.iloc[2] == '最下級').any()
    zone = list(dict_input.keys())[0]
    # print(list(dict_input.keys())[0])
    # return zone
    ...
    if condition_base in ['最上級', '不問']:
        ...
        # list_option_from_filtered_df_up = list(filtered_df_up.iloc[0, :].drop_duplicates().dropna())
        ...
        if '最上級' in list_condition:
            ...
            condition = '不問'
        # return set(list_option_from_filtered_df_up)
    ...
    if not any({**result_dict_up, **result_dict_down}.values()):
        return {}
    return {**result_dict_up, **result_dict_down}


# if __name__ == '__main__':
#     link_karen2 = r"C:\Users\KNT21617\Downloads\New folder (5)\NO1_FINAL_4\data\WZ1J\関連表2_I(WHハーネス)_XX4.xlsx"
...
#     df_input = df_karen2.iloc[1:4, 33:64]
#     dict_input_1 = {'us_最下級': ['conf-001', 'conf-002', 'conf-003']}
#
#     dict_input = {'us_不問': ['conf-003']}
#     time1 = time.time()
#     dict_output = check_option_new_v4(df_karen2, data_spec, df_input, dict_input_1)
#     print(dict_output)
